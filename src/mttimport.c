...
static gint get_file_type_by_signature(gchar *path_to_file)
{
  ...
  /* we compute the size before dynamically allocate buffer */
   ...
   if (strncmp (buffer, (const gchar*) &gtk_rtf_sign,21) == 0) {
      return iGtkRtfFile;
   }
   if (strncmp (buffer, (const gchar*) &write_sign,2) == 0) {
      return iMsWriteFile;
   }
   if (strncmp (buffer, (const gchar*) &old_word_sign,2) == 0) {
      return iOldMSWordFile;
   }
   if (strncmp (buffer, (const gchar*) &ole_sign,8) == 0) {
      return iOleMsWordFile;
   }
   if (strncmp (buffer, (const gchar*) &rtf_sign,4) == 0) {
      return iRtfFile;
   }
   if (strncmp (buffer, (const gchar*) &pdf_sign,4) == 0) {
      return iPdfFile;
   }
   if (strncmp (buffer, (const gchar*) &zip_sign,4) == 0) {
   ...
   if (strncmp (buffer, (const gchar*) &abiword_sign, 4) == 0)
    ...
    switch (buffer[i])
     {
       case '\\':{
         ...
         switch (buffer[i]) {
            ...
            default: {
				posmem = i;/* printf ("Mot clef à %d:", posmem);*/
...
************************************/
void on_merge_clicked (GtkButton *button, APP_data *data_app)
{
  GKeyFile *keyString;
  ...
  if (ret == GTK_RESPONSE_OK) {
    ...
    if (sign ==iGtkRtfFile) { 
       printf ("* Redac : OK, signature Gtk Rtf \n");
       ...
       if (inputFile == NULL) {
          printf ("* Redac : ERROR ! Can't open for merging with Redac file:%s *\n", filename);
          return;
       }
       /* we compute the size before dynamically allocate buffer */
       glong prev = ftell (inputFile);   
...
************************************/
void on_import_clicked (GtkButton *button, APP_data *data_app)
{
  GKeyFile *keyString;
  GtkTextBuffer *buffer;
  gchar *path_to_file, *filename;
